...
import { MWASheet } from './src/components/MWASheet';
import { HireModal } from './src/components/HireModal';
import { Inft, MOCK_INFTS } from './src/data';
...
// Navigation stack frame matches the prototype: each push is a kind + payload.
type StackEntry =
  | { kind: 'whispers' }
  | { kind: 'codex' }
  | { kind: 'inft'; inft: Inft }
...
type MwaState = { action: string; onApprove: () => void } | null;
...
export default function App() {
  ...
  const [mwa, setMwa] = useState<MwaState>(null);
  ...
  if (!splashSeen) {
    body = (
      <SplashScreen
        onEnter={() => {
          setMwa({
            action: 'Sign a connection challenge to enter the realm.',
            onApprove: () => {
              setMwa(null);
              setSplashSeen(true);
    ...
    body = (
      <StrategyEditorScreen
        ...
        onSave={() => {
          setMwa({
            action: `Update strategy for ${editingInft.name}. Effective at next tick. Cost: 1 GOLD.`,
            onApprove: () => {
              setMwa(null);
              pop();
              showToast('Strategy committed. Effective at next tick.');
            },
          });
        }}
      />
    ...
    if (tab === 'hearth') {
      const heroInft = MOCK_INFTS[0];
      body = (
        <HearthScreen
          inft={heroInft}
          onEnterCockpit={() => enterCockpit(heroInft)}
  ...
  // Tab bar visible only on root tabs.
        ...
        <View style={{ flex: 1, position: 'relative' }}>
          ...
          <HireModal
            ...
            onConfirm={() => {
              ...
              setHire(null);
              setMwa({
                action: `Hire ${hiredInft.name} for ${hiredInft.hireFee}.`,
                onApprove: () => {
                  setMwa(null);
                  showToast(
                    `${hiredInft.name} hired. Strategy is locked for the season.`
                  );
                  pop();
                },
              });
          ...
          <MWASheet
            visible={!!mwa}
            action={mwa?.action ?? ''}
            onCancel={() => setMwa(null)}
            onApprove={() => mwa?.onApprove()}
          />
        </View>
      </SafeAreaView>
    </SafeAreaProvider>
